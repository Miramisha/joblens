...
def database():
    ...
    assert state.is_relative_to(root/'.wrangler'), 'Test DB must stay under local .wrangler'
...
def challenge(email=None, code='123456', expired=False, attempts=0):
    email=email or os.environ.get('TEST_LOGIN_EMAIL') or secrets.token_hex(12)+'@example.test'
